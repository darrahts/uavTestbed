...
airframe = block.DialogPrm(1).Data;
thrust_constant = block.DialogPrm(3).Data;
torque_constant = block.DialogPrm(4).Data;
...
I_xx=airframe.Jb(1,1);
I_yy=airframe.Jb(2,2);
I_zz=airframe.Jb(3,3);

% thrust generated by rotors
T=(thrust_constant .*(w.^2));
T_total=round(sum(T),2);

% moment of each rotor
M=(torque_constant .*(w.^2));

% torque generated by rotors
Tau_x=airframe.l*(T(1)+ (sqrt(2)/2)*T(2)+ (sqrt(2)/2)*T(8) ...
    - T(5)- (sqrt(2)/2)*T(4)- (sqrt(2)/2)*T(6));%Tau_roll

Tau_y=airframe.l*(T(7)+ (sqrt(2)/2)*T(6)+ (sqrt(2)/2)*T(8) ...
...
% old
vdot_b=[0 ;0; -m_t*9.81]+R_b_e*[0; 0; T_total] - Dist_F/airframe.mass; % Linear Acceleration
dU=vdot_b(1); 
dV=vdot_b(2); 
dW=vdot_b(3);
...
ang_acc_x=((I_yy-I_zz)/I_xx)*q*r+(airframe.l/I_xx)*Tau_x; % - Dist_F(1)/model.mass;
ang_acc_y=((I_zz-I_xx)/I_yy)*q*r+(airframe.l/I_yy)*Tau_y; % - Dist_F(2)/model.mass;
ang_acc_z=((I_xx-I_yy)/I_zz)*q*r+(airframe.l/I_zz)*Tau_z; % - Dist_F(3)/model.mass;
...
%% additional outputs
block.Dwork(1).Data = double(vdot_b);
